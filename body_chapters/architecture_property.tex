...
\PM{Add an LDBC SNB schema figure (or compact table) here showing vertex types (Person, Post, Comment, Forum), their fields, types, and byte sizes. This anchors the running example used throughout the chapter: the 153-byte Person struct, the column-group assignments, and the write-amplification arithmetic all reference these fields.}

Before describing these modes, we must answer a prior question: how does the database determine which schema applies to a given vertex?
...
We assume that each pair of endpoint labels maps to exactly one edge label, a condition that holds in the LDBC Social Network Benchmark~\cite{erling2015ldbc} schema.
...
The key point is that omitting a \code{get\_value()} call on a column group is not sufficient to avoid I/O: \wt{} loads entire pages into its buffer pool regardless of whether the application reads the value.
...
The central design principle is to \textbf{separate fixed-size, scan-friendly columns from variable-length ones}: fixed-size groups yield high row density per page, while isolating variable-length columns keeps large blobs out of the buffer pool during narrow scans.
...
This section provides a qualitative comparison; we quantify the tradeoff experimentally in \autoref{sec:eval:property}.
