...
We integrate \project{} into the AsterDB evaluation artifact~\cite{moAsterEnhancingLSMstructures2025}, a reproducible Docker-based benchmark suite comparing seven graph storage systems on structural and property graph workloads: Neo4j, ArangoDB, JanusGraph, OrientDB, PostgreSQL (via SQLG), NebulaGraph, and AsterDB.
The AsterDB artifact uses Apache TinkerPop's Gremlin traversal language~\cite{apachetinkerpop} as a uniform query interface: each graph database implements a TinkerPop \emph{provider} that translates Gremlin traversals into its native storage operations.
The uniform interface comes at a cost that the artifact does not account for, and that we quantify in \autoref{sec:eval:microbenchmarks}: the provider path and the harness configuration together dominate the reported property mutation latencies.
\project{} bypasses this layer entirely, running all benchmarks through its native C++ API.
We evaluate \project{}'s property graph capabilities in three experiments of increasing scope.
First, we measure individual structural and property operations in isolation~(\autoref{sec:eval:microbenchmarks}).
Second, we benchmark 14 composite queries that combine structural traversal with property access, derived from the LDBC Social Network Benchmark (SNB)~\cite{erling2015ldbc} at scale factor~3~(\autoref{sec:eval:property-ldbc}).
The dataset contains ${\sim}800$\,K vertices and ${\sim}3.5$\,M edges across 4~entity types and 7~relation types (schema in \autoref{fig:ldbc-snb-schema}).
Third, we run iterative algorithms on the full cit-Patents graph and compare \project{} to graph database baselines~(\autoref{sec:eval:property-graphalytics}).

\input{body_chapters/eval_property/eval_microbenchmarks.tex}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{LDBC SNB Query Performance}
\label{sec:eval:property-ldbc}

The microbenchmarks above isolate individual primitives; we now examine how those primitives compose under realistic query workloads.
...
We exclude AsterDB from this comparison: although its RocksDB backend persists structural data, its Gremlin bridge keeps vertex properties in an in-memory map that is never populated from storage, so property reads return null and property writes never reach storage; additionally, all edges are stored under a single default label, preventing label-filtered traversals.
...
\begin{table}[h]
  ...
  \caption{LDBC SNB query p50 latency, scale factor~3.
    ...
    (1000 queries per request) to amortize per-request overhead.
...
Neo4j and ArangoDB operate as client-server systems, so every individual query carries a request round-trip, query parsing, and result serialization.
Measured single-shot, Neo4j R1 takes $1.95$\,ms, of which ${\approx}$98\% is per-request and query-parse overhead: Neo4j's no-op round-trip alone costs ${\approx}$1.5\,ms.
NeuG and \project{}, by contrast, are embedded stores with no network layer.
To avoid penalizing Neo4j and ArangoDB for an architectural choice orthogonal to query-engine performance, we batch 1,000 parameterized queries in a single \texttt{UNWIND} request (Cypher) or \texttt{FOR-IN} loop (AQL) and divide by the batch size, amortizing per-request overhead to $<$3\,$\mu$s.
...
R1 returns a Person's seven~scalar properties plus multi-valued email and language
...
R1 completes in $7$\,$\mu$s on \project{}, $3.9\times$ faster than Neo4j ($28$\,$\mu$s), $2.7\times$ faster than ArangoDB ($20$\,$\mu$s), and $110\times$ faster than NeuG ($802$\,$\mu$s).
...
\project{}'s native C++ API trades the convenience of a declarative query language for direct storage-engine access.
The cost of that layer is large: on property CRUD operations, the TinkerPop path and its cold-start costs together account for 92--96\% of the latency gap between \project{} and the two baselines we decompose, Neo4j and ArangoDB (\autoref{tab:crud-overhead}).
On iterative algorithms, Gremlin framework and index-resolution overhead consume 50\% of Neo4j's per-iteration cost (\autoref{sec:eval:property-graphalytics}).
In practice, graph queries are rarely written ad hoc; they are embedded in application logic and invoked programmatically~\cite{10.1145/3357766.3359541,cheung2013optimizing}.
...
%
X3 favors \project{} at $0.24$\,ms, $4.6\times$ faster than NeuG ($1.1$\,ms) and $55\times$ faster than Neo4j ($13.1$\,ms).
IC-9 also favors \project{} ($1.5$\,ms) over Neo4j ($65.4$\,ms, $44\times$) and ArangoDB ($188.8$\,ms, $126\times$).
However, IC-3 reverses the ranking: NeuG completes in $3.8$\,ms, Neo4j in $7.6$\,ms, and \project{} in $29.0$\,ms.
...
In columnar mode, BI-1 takes $404$\,ms, $4.4\times$ faster than Neo4j ($1{,}783$\,ms) and $16.5\times$ faster than ArangoDB ($6{,}666$\,ms); NeuG takes $892$\,ms.
...
%
ArangoDB and Neo4j dominate W1--W3 ($21$--$68$\,$\mu$s), because their write paths are optimized for single-document upserts (insert key if not exists, update if it does) through mature, heavily tuned storage engines.
...
NeuG takes ${\sim}$190\,ms on W1 and W2, three orders of magnitude slower than every other system, likely reflecting the cost of mutating its mmap-based CSR representation, which is optimized for read traversals.

\section{Embedded vs.\ Columnar Trade-off}
\label{sec:eval:emb-vs-col}
...
For multi-hop traversals that access properties at each hop, embedded mode wins decisively: IC-9 takes $1.5$\,ms embedded vs.\ $17.7$\,ms columnar ($12\times$); IC-5 (not shown) takes $2.65$\,ms vs.\ $15.2$\,ms ($5.7\times$).
...
Because each vertex or edge label has its own property table, the choice between embedded and columnar storage is not global: the operator assigns each property to the mode that matches its access pattern at schema-definition time.
Properties accessed during multi-hop traversals (e.g., \code{creationDate} for date-range filters) belong in embedded storage; properties scanned in bulk for analytics (e.g., \code{length} for BI-1) belong in column groups.
A property that serves both roles can be stored in both representations, trading additional write cost for read performance on both paths.

\section{Graph Algorithms}
\label{sec:eval:property-graphalytics}

The experiments above measure per-operation and per-query latency on moderately sized property graphs.
We now scale to cit-Patents (3.77\,M vertices, 16.5\,M edges), a structural graph an order of magnitude larger than the LDBC SNB dataset, and test full-graph iterative algorithms against the same baselines.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[h!]
  \centering
  % \footnotesize
  ...
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lrrrrr@{}}
    ...
    \bottomrule
  \end{tabular*}
  \caption{Graph algorithm runtimes (s) on cit-Patents. PR, CDLP, WCC are full-graph iterative; BFS, SP are single-pair. t/o~=~exceeded the 2\,h timeout.}
  \label{tab:graph-algorithms}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\autoref{tab:graph-algorithms} reports algorithm runtimes.
...
ArangoDB and JanusGraph exceed the 2-hour timeout on all iterative algorithms but complete the single-pair traversals quickly (BFS in 30 to 100\,ms, SP in 30 to 110\,ms) where their native shortest-path implementations apply.
...
(1)~\emph{no-op}, which measures the per-call cost of the TinkerPop traversal machinery with no graph access;
