...
We integrate \project{} into the AsterDB evaluation artifact~\cite{moAsterEnhancingLSMstructures2025}, a reproducible Docker-based benchmark suite comparing seven graph storage systems---Neo4j, ArangoDB, JanusGraph, OrientDB, PostgreSQL (via SQLG), NebulaGraph, and AsterDB---on structural and property graph workloads.
...
We quantify the overhead of this layer in~\autoref{sec:eval:property-ldbc}; \project{} bypasses it entirely, running all benchmarks through its native C++ API.
...
The cost of that declarative layer is large: on property CRUD operations, Gremlin parsing and traversal compilation account for 93--96\% of the latency gap between Neo4j and \project{} (\autoref{tab:crud-overhead}).
...
\begin{table}[h!]
  ...
  \begin{tabular*}{\columnwidth}{@{\extracolsep{\fill}}lrrrrr@{}}
    ...
    \bottomrule
  \end{tabular*}
  \caption{Graph algorithm runtimes (s) on cit-Patents. PR, CDLP, WCC are full-graph iterative; BFS, SP are single-pair. Timout ~=~2\,h.}
