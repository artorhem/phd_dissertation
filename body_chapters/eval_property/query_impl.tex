...
We used Claude (Anthropic) Opus 4.6 \cite{anthropic2026claudecodedocs} to generate these implementations in a three-stage pipeline: schema interface generation, query logic synthesis, and cross-system verification.
We provided the LLM with the LDBC SNB Interactive v1 query specifications and the \project{} source code (header files defining the \code{GraphBase} API, cursor classes, and property schema namespaces) as context.
\PM{search and put context in appendix.}
...
The Place and Organisation hierarchies (City/Country/Continent and Company/University) each receive distinct enum values rather than a shared one.
In the LDBC SNB schema, \textsc{studyAt} targets only University and \textsc{workAt} targets only Company; collapsing both into a single Organisation type would produce ambiguous \code{(src\_type, dst\_type)} pairs (both Person$\to$Organisation), preventing edge-type inference from endpoint labels alone (\autoref{arch:label-encoding}).
...
For each query, we provided the LLM with the specification's input parameters, output schema, sort order, and limit, along with the schema interfaces from the first stage.
