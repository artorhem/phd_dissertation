...
\begin{enumerate}
  ...
  \item \textbf{In-memory graph analytics} (\autoref{sec:eval:inmem}).
        When memory is sufficient to hold the entire graph, can \project{}
        match systems that exploit this assumption by building
  ...
  \item \textbf{Property graph queries} (\autoref{sec:eval:property}).
        ...
        We compare against Neo4j, ArangoDB, NeuG, AsterDB, JanusGraph,
        OrientDB, and PostgreSQL on individual
...
capacity enable scalability testing.
...
We compile all \project{} code and comparison systems with GCC~13.3.0
...
(${\sim}317$\,K vertices, ${\sim}1$\,M edges) and WikiTalk (${\sim}2.4$\,M vertices, ${\sim}5$\,M edges), and two property graphs from the AsterDB artifact, LDBC and Freebase.
...
The out-of-core (\autoref{sec:eval:ooc}) and in-memory
(\autoref{sec:eval:inmem}) experiments use LDBC Graphalytics benchmark
kernels~\cite{LDBCGrapalytics22:online} (BFS, PageRank, connected components,
triangle counting, and SSSP), each implemented using the system's native
API\@.
...
The LDBC SNB queries are additionally compared to
