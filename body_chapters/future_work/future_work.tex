...
\project{} currently exposes a C++ cursor API.
...
For an embedded database this is the natural interface: applications generate queries programmatically, and the cursor API gives them fine-grained control over access paths.

Nonetheless, a declarative query language layer would broaden \project{}'s applicability by letting users express graph queries without specifying how to traverse the underlying tables.
The natural target is GQL, the ISO standard for property graph queries~\cite{francis2023gql-standard}, or openCypher~\cite{openCypher}, which is already supported by Neo4j, Memgraph, and several other systems.
...
A second question is whether a language model can bypass the need for a bespoke query language altogether.
Recent work on LLM-based text-to-GQL and text-to-Cypher translation~\cite{mandilara2025text2cypher, lyu2026text2graphquerybench} demonstrates that language models can generate syntactically correct graph queries from natural language.
Mason~\cite{mason2025indaleko} takes this further in Indaleko, a unified personal index that uses natural language interfaces backed by LLMs to generate graph queries and API calls against an underlying graph store, eliminating the requirement for users to learn a graph query language at all.
Applying a similar approach to \project{} would require the model to select between AdjList and EdgeKey access paths, choose between embedded and columnar property reads, and decide on scan direction (in-neighbors vs.\ out-neighbors) based on the workload context.
...
A central claim of this dissertation is that data layout is the primary performance determinant for graph analytics (\autoref{chap:design_space}).
...
This is the graph-property analogue of the column-group selection problem in column stores~\cite{Rasin2013colstoreselection}, and query co-access frequency matrices provide the input signal for optimization.
...
Graph Neural Networks (GNNs) have become the dominant approach for machine learning on graph-structured data, with applications in fraud detection, recommendation systems, drug discovery, and knowledge graph completion~\cite{hamilton2017graphsage, velivckovic2017gat, kipf2016gcn}.
...
DiskGNN~\cite{Liu2025DiskGNN}, GNNLab~\cite{Yang2022GNNLab}, MariusGNN~\cite{Waleffe2023MariusGNN}, ByteGNN~\cite{Zheng2022ByteGNN}, and BGL~\cite{liu2023bgl} address neighbor sampling and feature loading from disk, but all assume a static graph snapshot loaded before training begins.
...
The PyTorch Geometric (PyG) framework defines two abstract interfaces, \texttt{FeatureStore} for node properties as tensors and \texttt{GraphStore} for edge indices in COO format, that enable a graph database to serve as a remote backend for GNN training~\cite{chen2023kuzu-pyg-backend}.
...
An in-process side-car HNSW index~\cite{malkov2020hnsw}, rebuilt from a \wt{} snapshot and atomically swapped into service, would provide sub-millisecond ANN queries with snapshot-consistent semantics.
