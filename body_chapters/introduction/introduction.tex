...
\paragraph{Graphs Are Ubiquitous.}
\label{sec:intro:ubiquitous}

New data are being generated rapidly. There is an ever-present and growing need to store and analyze them efficiently.
Much of these data have some inherent schema or structure, and relational databases are a natural choice for efficient storage.
...
Graphs are a natural way to model such relationships, and are widely used to to model interactions on social networks, hyperlinks on the World Wide Web, recommendation systems, financial transactions for fraud detection, and protein interactions in biological studies~\cite{ubiquityLargeGraphsVLDB2017}.
...
These large graphs are now ubiquitous. More than a decade of research has investigated how to store and process them efficiently.
...
\paragraph{Graph Workloads: OLTP vs.\ OLAP}
...
\newflag[new section]
\textbf{Graph analytics and graph transactions do not require separate systems.}
A single persistent store can serve both.
The key is choosing an analytics-friendly data layout and managing properties independently of the structure of the graph.
That independence lets each label choose the representation its access pattern demands, rather than imposing one choice on the whole graph.
A property can be inline with the record to which it belongs or in a column group scanned in bulk.
Analytics-friendly layouts can be encoded as sorted key ranges in a general-purpose transactional key-value store.
Such a store needs no ETL and no preprocessing pass; the layout is always ready for analysis.
We demonstrate that such a system remains competitive with purpose-built systems across the full operating spectrum of graph data management: in memory and out of core, static and dynamic, structural and property graphs.

\section{\project{}: Our Approach}
\label{sec:intro:flexograph}

This dissertation presents the design of \project{}, a flexible and performant \ac{HGTAP} database system.
...
Insertion throughput scales near-linearly to 72 threads, sustaining up to 1.48\,M edges/sec on the write-optimized layout (\autoref{fig:dynamic-scaling}), while property-mutation latencies are three to four orders of magnitude lower than those of Neo4j~\cite{neo4j} and JanusGraph~\cite{janusgraph}.

A programmer uses \project{} through a library API that provides read-only snapshot iterators over neighborhoods and properties to express analytics tasks.
The API exposes both neighborhood and edge-stream iterators, so the same algorithm can be written as a vertex program or an edge program and run against either representation.
...
\begin{itemize}
  ...
  \item A controlled study of the coupling between physical representation and computation model.
  Because \project{} stores both representations behind a single API, we run vertex-centric and edge-centric implementations of the same algorithms over both layouts on one storage engine, isolating representation as the only variable.
  Each representation has a natural partner, and mismatching them costs up to $6.2\times$ more cursor operations (\autoref{sec:eval:ec_vc}).
...
\begin{figure}[t]
  \centering
  \includegraphics[]{fig/spider_chart_v2.pdf}
...
The chapter takes a look at some strategies that can be used for handling data that is larger than installed memory, and concludes with a brief introduction to the \wt{} storage engine and all the internal concepts that inspire the \project{} architecture and are critical to understanding performance.
...
Since \project{} can \emph{flexibly} operate under conditions that vary along these axes, it is important to analyze some exemplar systems along several points in the space.
We particularly focus on the systems to which we compare \project{}'s performance.
...
\autoref{sec:eval:static} compares \emph{static structural graph} analytics performance to both in-memory and out-of-core systems.
\autoref{sec:eval:dynamic:chapter} measures insertion scalability and post-ingestion analytics on \emph{dynamic structural graphs} comparing to other in-memory systems.
\autoref{sec:eval:property:chapter} evaluates \emph{property graph} workloads using the LDBC Social Network Benchmark to compare to Neo4j, ArangoDB, and JanusGraph.
