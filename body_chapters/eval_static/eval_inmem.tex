...
We then measure the total time to run the four graph kernels that all five systems implement: PageRank, Connected Components, BFS, and Betweenness Centrality.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{fig/in-mem/flexograph_inmem_results.pdf}
  \caption{In-memory graph analytics performance. The hatched portions of each bar show the preprocessing time.}
...
\begin{table}[h]
  ...
  \begin{tabular}{lrrrrr}
    ...
    \bottomrule
  \end{tabular}
  \caption{Peak memory usage (GiB) of in-memory systems.}
...
Ligra consistently consumes memory that is $\approx$3$\times$ the graph size, so the Linux OOM killer terminates it on graph500-30.
...
GapBS is also relatively memory efficient consuming less memory than \project{} on five of the six graphs.

\paragraph{Takeaway:} \project{} avoids costly ETL thanks to its ability to run directly on its persistent representation.
This benefit comes at slightly increased memory consumption relative to the lightest competitors and decreased performance if the other systems are able to retain an in-memory representation between analytics runs.