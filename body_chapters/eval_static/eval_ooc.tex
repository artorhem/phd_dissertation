\section{Out-of-Core Graph Analytics}
\label{sec:eval:ooc}
...
To force out-of-core behavior, we cap each container at 50\% of the graph's raw on-disk edge-list size with swap disabled. Because the layouts differ, we do not tie this budget to any single system's in-memory footprint, thus privileging systems whose in-memory representation is most compact.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{fig/ooc/flexograph_ooc_results.pdf}
  \caption{Out-of-core graph analytics. Solid bars: combined execution time for both PageRank + CC. Hatched: preprocessing time. Note the log Y axis; preprocessing time often dominates runtime for the systems that need to create specialized layouts from an edgelist.}
...
\project{}'s preprocessing cost is negligible (opening a read-only cursor on the latest checkpoint and partitioning work across threads) because it operates directly on its persistent representation rather than building a single-purpose on-disk layout from an edge list (\autoref{design:static_structural}).

\begin{figure}[h]
  \centering
  \includegraphics[width=\columnwidth]{fig/ooc/iostat_read_bandwidth.pdf}
  \caption{Read BW vs.\ time for PR on graph500-26. Dashed line marks 540\,MB/s device peak sequential read bandwidth.}
...
X-Stream achieves higher peak bandwidth ($\approx$480 MB/s), but exhibits a pronounced sawtooth pattern: bursts of sequential edge streaming alternate with idle periods during its gather-and-shuffle phase, pulling the average down to 330 MB/s.
