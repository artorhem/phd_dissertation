...
A preprocessing pipeline transforms the raw edge list into a format compatible with \project{}'s representations.
The input is a CSV edge file sorted by source vertex ID.
The pipeline makes two passes over the data and sorts each reverse-edge chunk in parallel:
...
When the buffer reaches a configurable size (default 10\,GB, approximately 625M edges), the program sorts the chunk with \texttt{tbb::parallel\_sort}\cite{onetbb_parallel_sort} and flushes it to a binary run file on disk.
...
\[
  \texttt{[node\_id}^{64} \;|\; \texttt{degree}^{32} \;|\; \texttt{dst\_id}_1 \;|\; \cdots \;|\; \texttt{dst\_id}_{d}\texttt{]}
\]
Subscripts denote indices, superscripts denote bit widths, $d$ is the degree, and each field appears in its native binary representation.
...
The pipeline partitions output files by vertex ID range: $\text{partition}(v) = \lfloor v \cdot N / |V| \rfloor$, where $N$ is the number of partitions (typically matching the core count).
No adjacency list spans a partition boundary, because its source vertex assigns it to exactly one partition, and so the loader never needs a cross-partition merge pass.
...
Multiple threads can insert into the same table concurrently, because the loader sets no \texttt{bulk} configuration.
This loader is the simplest to implement and works well for small and medium graphs, but transaction overhead and write-ahead logging make it slower than the bulk-cursor approaches below.
...
The bulk cursor bypasses transaction overhead and writes pages directly, but imposes two constraints: (1)~the caller must insert keys in sorted order, and (2)~a table admits only one open bulk cursor at a time, which rules out multi-threaded insertion into that table.

\project{} uses the standard bulk loader by streaming presorted partition files through a single bulk cursor per table.
\project{} loads the tables one at a time so that each pass has the full \wt{} cache to itself, which maximizes page-write throughput.
...
To remove the single-threaded constraint, \project{} modifies the \wt{} bulk loader so that several threads can insert into one table concurrently through bulk cursors.
...
Leaf-page creation therefore needs no inter-thread synchronization beyond the file ID allocation, because each thread writes only to its own backing file.
...
Each thread then contributes its final root-level key--address pairs to a shared array that a spinlock guards.
The last thread to finish, which an atomic decrement of the open-cursor count identifies, sorts all root-level keys and builds the single root page of the B-tree.

This design parallelizes the I/O-intensive leaf-page writing phase and confines synchronization to spinlock-protected metadata operations in the merge phase.
We originally built the modifications against \wt{} 3.2.1 and have since ported them to the \wt{} 11.0.0 release that the rest of \project{} uses.
...
The test machine has a 14-core Intel Xeon W-2275 at 3.30\,GHz (28 hardware threads), 128\,GB of RAM, and a conventional hard disk (Machine B in \autoref{sec:eval:env}).
...
Adjacency-list streaming through a single bulk cursor took 301 seconds for the in- and out-neighborhood tables combined.
...
Edge-table streaming ran slightly slower in the parallel case (81\,s versus 53\,s). For fine-grained per-edge keys, the hierarchical merge that combines each thread's B-tree into a single root adds enough work to offset the parallel leaf-page writes.
\begin{table}[ht!]
\centering
\begin{tabular}{lr}
\toprule
...
All three loaders read the same preprocessed partition files, so they share the preprocessing cost (\autoref{arch:bulk-preprocess}).
Table~\ref{tab:preprocess-times} breaks down the two preprocessing passes on the same machine with 16 threads.
The sequential CSV read and the forward-adjacency-list writes dominate Pass~1; the two \texttt{tbb::parallel\_sort}~\cite{onetbb_parallel_sort} calls on the reverse-edge chunks account for only 16 of its 310 seconds.
