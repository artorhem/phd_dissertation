...
Structure and properties are distinct kinds of data that queries consume independently more often than together: graph analytics such as PageRank and BFS traverse only topology, while point lookups and column scans read only attributes.
This disjointness justifies storing each in a layout tuned to its access pattern, without splitting the system into separate stores.
...
Second, in the subset of the LDBC SNB schema we target, each pair of endpoint labels maps to exactly one edge type (\textsc{knows} connects only Person$\to$Person, \textsc{hasCreator} connects only Post$\to$Person), so edge labels can be inferred from endpoint labels without an explicit label column.
This property does not hold in general: multiple edge types may connect the same vertex-type pair. 
Supporting the general case requires adding an edge-type field to the key, a straightforward extension we omit because our target schema does not need it.
...
Traversing that edge requires a foreign-key-like lookup into the destination label's table, an operation that is difficult to batch and that breaks sequential scan patterns.
\autoref{fig:label:two-reps} illustrates the problem: iterating through the neighborhood of a Post vertex in the Post table requires repeated random lookups into the Person table to resolve each \texttt{hasCreator} destination, much as chasing foreign keys in a normalized relational schema does.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig/architecture_property/alternative1_duplicate.pdf}
...
\begin{figure}[]
  ...
  \begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.75]{fig/architecture_property/alternative2a_embed.pdf}
    \caption{EdgeKey}
  ...
  \begin{subfigure}[b]{\textwidth}
    \centering
    \includegraphics[scale=0.75]{fig/architecture_property/alternative2_embed.pdf}
...
An initial prototype implemented this approach as a user-space framework that re-mapped original vertex IDs into partitioned ranges and maintained a bidirectional mapping table.
\autoref{fig:label:relabel} illustrates the re-labelled ID space.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\textwidth]{fig/architecture_property/alternative3_relabel.pdf}
...
This encoding limits vertex types to $2^{8} = 256$ and vertices per type to $2^{56}$.
The per-type capacity is large enough to be irrelevant in practice, but a schema with hundreds of vertex types would exhaust the tag space; widening the tag field to 16 bits requires only a compile-time constant change.
