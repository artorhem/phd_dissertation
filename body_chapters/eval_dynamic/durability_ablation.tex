...
periodic checkpointing; each is a binary decision producing the four configurations in
...
at 800\,GiB so that the graph is resident throughout, producing no eviction pressure; degree caching is disabled; memory-mapped I/O and statistics collection
...
\autoref{tab:durability-ablation} presents the results. \project{}'s full
...
elsewhere in this chapter, including those where the gap relative to the in-memory
...
The remaining cost cannot be eliminated. Disabling the log and
