...
insertions and ${\sim}$45\% deletions) using \project{} SplitEdgeKey with 72
...
We compare two configurations that differ \emph{only} in the four configured eviction
thresholds of \autoref{tab:wt-eviction-knobs}: our own setting (blue) and WiredTiger's
defaults (red). Because no checkpoint is running, this isolates the effect of the
...
arms produce qualitatively different behavior.
...
We stopped this arm after 201~minutes, at which point it had completed only 61\% of the update log having written 18{,}657\,GiB (117$\times$ what \project{}'s setting wrote for the \emph{entire} workload, or ${\sim}$191$\times$ normalised per unit of work).
