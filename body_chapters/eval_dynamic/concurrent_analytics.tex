...
\emph{Update bytes} counts only the in-memory update chains that \project{} appends to on every edge insertion, not the page images that hold them.
...
Once analytics begins, the log deletes edges about as fast as it inserts them, so the graph's size changes by less than $0.5\%$ across the analytics window.
...
Two structural changes would attack the problem at its source: sharding the edge table so that the checkpoint bars eviction on one shard at a time, and a checkpointer that parallelises within a single tree.
