...
Producing that checkpoint means writing out the B-trees that hold the graph, and the writers keep modifying those same trees while they are  written.
...
\begin{figure}[!htb]
  ...
  These explicit checkpoints reduce writer throughput to a mean of $290$\,K\,ops/s in the first pass and $95$\,K\,ops/s in the second, and the throughput recoveries between checkpoints are the only windows in which the writers get work done.
  ...
  The two runs also start from different pre-analytics rates, which the policy cannot account for because it takes effect only once analytics begins; read each curve against its own dashed line rather than against the other panel.}
...
The first thing we tried was giving the checkpoint less to contend with.
Each writer thread adds to the dirty data that a checkpoint must write out and that eviction must reclaim, so cutting the count from $25$ writers to $5$ slows the rate at which it builds up.
The rest of this section runs at $W=5$ unless stated otherwise.
This approach only buys time; the first analytics pass that stalls the writers now arrives twenty-eight minutes into the analytics window rather than three.
...
Setting $N=0$, as the last row of \autoref{tab:concurrent-space} suggests, stops the background server, so no checkpoint ever runs, nothing bars eviction, and the writers keep running: they retain $66.7\%$ over $57$ minutes and stall for $0.7\%$ of that time.
...
At $W=5$ with $N=120$\,s, checkpoints occupy $64\%$ of the wall-clock instead of the $94\%$ they occupy at the $10$\,s default, and the update bytes that trigger conscription peak at $77.2$\,GiB without ever reaching the $80$\,GiB threshold where the $10$\,s run pins.
...
The $30\%$ they each lose measures the cost of running analytics alongside writers rather than the cost of checkpointing, because two designs with nothing in common land within a percentage point of each other.
...
Two structural changes would attack the problem at its source: sharding the edge table so that the checkpoint bars eviction on only one shard at a time, and a checkpointer that parallelises within a single tree.
