...
GraphOne and Teseo are substantically faster than \project{}, sustaining 5.6--6.6 and 4.3--5.5\,M~edges/s respectively, 3 to 5$\times$ faster than \project{} EdgeKey (1.2--1.8\,M~edges/s).
...
Teseo sustains 7.0\,M~ops/s, which is $\approx$30\% higher than its insertion-only rate! This can be attributed to updates creating space for new insertions in an update-in-place data structure such as the Teseo tree.
...
Absent high-degree vertices, concurrent appends spread evenly across adjacency records rather than concentrating on a few hot lists, eliminates the contention that penalised the larger thread pool on the power-law graph.
...
\begin{table}[t]
  ...
  EdgeKey SF-26 entries are absent because those runs were not collected.}
  \label{tab:cold-open}
\end{table}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The critical difference is what the cost buys.
...
Teseo produces the best BFS runtime: 0.61\,s on graph500-26, but its PageRank and WCC degrade sharply at scale---27\,s and 12\,s respectively---while \project{}'s PageRank is 5.7\,s, $4.7\times$ faster.
