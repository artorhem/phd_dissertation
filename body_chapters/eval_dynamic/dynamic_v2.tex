...
It excludes write-ahead logging and checkpointing, which are off unless noted otherwise.
...
\project{} is not competitive with the in-memory systems on raw insertion throughput, and the table states the gap bluntly: \project{} EdgeKey's peak of 1.69\,M~edges/s at 72~threads is exactly GraphOne's single-thread throughput.
At one writer \project{} trails GraphOne by 24$\times$; at peak it trails GraphOne by 3.7$\times$ and Teseo by 3.0$\times$.
The deficit is structural; \autoref{sec:dynamic:ingest-update-tput} accounts for where the time goes.
...
Absent high-degree vertices, concurrent appends spread evenly across adjacency records rather than concentrating on a few hot lists, which eliminates the contention that penalized the larger thread pool on the power-law graph.
