...
\begin{figure*}[ht]
  ...
  \caption{~Update (insert/delete workload) throughput on graph500-24 and uniform-24. GraphOne throughput collapses for this workload. \project{}'s AdjList works better at 72 threads on the uniform-24 workload becuase of reduced contention between threads.}
...
The largest shift is in Adjacency List: with 72 writer threads it reaches 0.58\,M~ops/s on uniform-24, outperforming its own 48-thread arm (0.46\,M~ops/s) and reversing the thread-count ordering seen on graph500-24, where 48 threads (0.28\,M~ops/s) beats 72 (0.18\,M~ops/s).
Absent high-degree vertices, concurrent appends spread evenly across adjacency records rather than concentrating on a few hot lists, eliminate the contention that penalised the larger thread pool on the power-law graph.
...
\begin{figure*}[h!]
  ...
  \begin{minipage}[t]{\columnwidth}
    ...
    \end{tabular}
    \vfill
    \captionof{table}{Cross-system Graphalytics kernel time (s), median over warm runs. The best time for each graph/algorithm appears in bold. FG = FlexoGraph, PR = PageRank.}
