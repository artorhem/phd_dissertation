...
Most dynamic graph systems (such as ASPEN~\cite{10.1145/3314221.3314598}, STINGER~\cite{stinger2012ediger}, Sortledton~\cite{sortledton2022Fuchs}, GTX~\cite{libinzhou2025GTX}, etc.) have no way to persist data across restarts; the exceptions are LiveGraph~\cite{zhu2019livegraph}, which supports file-backed mmap, and GraphOne~\cite{graphone2019kumar}, which provides a write-ahead log.
...
\begin{enumerate}
  \item A scalability study that varies writer thread count (\autoref{sec:eval:dynamic:scalability}).

  \item An insertion benchmark at each system's optimal thread count (\autoref{sec:dynamic:ingest-update-tput}).

  \item A sustained mixed insert-and-delete workload that replays a graph update log (\autoref{sec:dynamic:ingest-update-tput}).

  \item A graph analytics benchmark (PageRank, BFS, and weakly connected components) run on the graph each system holds once the insertion-only ingest of experiment~2 completes (\autoref{sec:eval:dynamic:analytics}).
\end{enumerate}
...
This gap is the persistence machinery tax defined in~\autoref{sec:eval:dynamic}.
...
Absent high-degree vertices, concurrent appends spread evenly across adjacency records, rather than concentrating on a few hot lists,  eliminates the contention that penalized the larger thread pool on the power-law graph.
...
The competing systems incur no comparable cost, because they require no reconciliation step: LiveGraph's first runtime is nearly indistingishable from  its warm runtimes, and Teseo adds only a few seconds of cache warm-up.
