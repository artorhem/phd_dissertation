...
% EDITED: the configuration is now introduced by the ablation that precedes
% this section, so this line back-references it instead of re-explaining it.
...
\autoref{sec:eval:dynamic:durability} measured the cost of the two mechanisms \project{} disables when a workload does not need persistence, write-ahead logging (\texttt{log=(enabled=false)}) and checkpoints (\texttt{checkpoint=(wait=0, log\_size=0)}); for the profile below we additionally raise all eviction thresholds to 100\% so that no eviction work occurs.
...
\paragraph{Persistence components are negligible.}
\label{sec:wt-overhead-persistence}
We used \texttt{perf} to profile the same ingest-only experiment we report in \autoref{sec:dynamic:ingest-update-tput}: graph500-24 ingestion at one and 72 writers. 
We then use DWARF\cite{dwarf5standard} to resolve each sample to its \wt{} source function.
...
With WAL and checkpoints disabled, the persistence components (reconciliation, block management, logging, and page splits) account for 46\,ns per inserted edge, under 0.5\% of writer CPU time.
This is a different measurement from the ablation, not a restatement of it: \autoref{sec:eval:dynamic:durability} prices turning durability \emph{on} ($1.67\times$ on ingest throughput), whereas the 46\,ns here is what persistence code costs while durability is \emph{off}. Neither quantity accounts for the gap.
...
\autoref{tab:wt-overhead} shows the per-edge breakdown (in ns) at one and 72 writers on graph500-24, with WAL and checkpoints disabled.

\begin{table}[h]
  \centering
  \begin{tabularx}{\columnwidth}{@{}Xrrrr@{}}
    \toprule
    Component & $T_1$ (ns) & \% of $T_1$ & $T_{72}$ (ns) & Growth ($T_{72}/T_1$) \\
    \midrule
    B-tree descent and insertion           & 5,421 & 39 & 6,664 & $1.2\times$ \\
    Key comparison during descent          & 2,180 & 16 & 2,603 & $1.2\times$ \\
    Cache accounting and eviction protocol & 1,047 & 8 & 18,879 & $18.0\times$ \\
    Cursor, transaction, config parsing    & 1,819 & 13 & 13,017 & $7.2\times$ \\
    Memory allocation and page faults      & 1,570 & 11 & 653 & $0.4\times$ \\
    Key serialization (packing)            & 185 & 1 & 264 & $1.4\times$ \\
    Persistence components                 & 46 & $<$1 & 62 & $1.3\times$ \\
    Other                                  & 1,554 & 11 & 3,520 & $2.3\times$ \\
    \midrule
    Writer on-CPU total & \textbf{13,822} & & \textbf{45,662} & $\mathbf{3.3\times}$ \\
    \bottomrule
  \end{tabularx}
  \caption{On-CPU time per inserted edge for \project{} on graph500-24 with WAL and
  checkpoints disabled.
  $T_1$ and $T_{72}$ are the on-CPU nanoseconds a component spends per inserted edge at one
  and at 72 concurrent writers.
  The third column gives each component's share of $T_1$, and the last gives the growth
  ratio $T_{72}/T_1$: a value above $1\times$ means the component spends more CPU per edge
  under contention than it does alone.
...
Any storage engine that maintains a sorted B-tree over variable-length keys will pay a comparable cost for descent and comparison; this is not specific to \wt{}'s persistence design.
...
\wt{} creates the eviction server thread unconditionally at connection startup; no configuration can disable it.
It runs a wake-check-sleep loop with an auto-scaling interval, but with thresholds at 100\%, it finds nothing to do, and returns to sleep.
The cost measured above comes from the per-access protocol that application threads execute, not the background thread.

\paragraph{API and transaction bookkeeping.}
Cursor setup and teardown, transaction management (snapshot walks over active sessions, transaction-ID allocation), and \wt{}'s configuration-string parser together account for 13\% at one writer.
Under contention these grow sharply: the cursor API alone reaches 6,346\,ns at 72 writers ($\times$23 its single-writer cost), and transaction bookkeeping grows to 5,169\,ns ($\times$13).
  ...
  \begin{tabularx}{\columnwidth}{@{}XrrX@{}}
    ...
    B-tree access path (descent, key comparison, transactions, cursor API, key packing)
      & $\sim$81\% & $\sim$51\% & Operating a transactional sorted index. Any storage
        engine with a B-tree, MVCC, and a cursor API will pay a comparable cost. \\
    \bottomrule
  \end{tabularx}
...
The B-tree access path is a cost any transactional sorted index imposes.
