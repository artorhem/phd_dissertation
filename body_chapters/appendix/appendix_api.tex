...
% ── Root ──
\node[base] (root) at ({1.5*\CS}, 0) {table\textunderscore iterator};
...
\draw[ext] ([xshift=-1.5mm]a1.west) -- ([xshift=-1.5mm]a2.west);
\foreach \n in {a1,a2}
...
\draw[ext] ([xshift=-1.5mm]b1.west) -- ([xshift=-1.5mm]b2.west);
\foreach \n in {b1,b2}
  \draw[ext] ([xshift=-1.5mm]\n.west) -- (\n.west);
...
\draw[ext] ([xshift=-1.5mm]c1.west) -- ([xshift=-1.5mm]c2.west);
\foreach \n in {c1,c2}
  \draw[ext] ([xshift=-1.5mm]\n.west) -- (\n.west);
...
\draw[ext] ([xshift=-1.5mm]d1.west) -- ([xshift=-1.5mm]d2.west);
\foreach \n in {d1,d2}
  \draw[ext] ([xshift=-1.5mm]\n.west) -- (\n.west);
\draw[inh] ([xshift=-1.5mm]d1.west) -- ([yshift=-0.5mm]edg.south west);

\end{tikzpicture}
\caption{Inheritance hierarchy for the topology cursor classes.  Each representation provides concrete implementations (e.g., \code{AdjOutCursor}, \code{EkeyOutCursor}) of the four abstract cursor types.}
