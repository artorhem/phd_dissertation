...
\newflag[rewritten; new comparison table]
...
\autoref{tab:static_systems} groups these systems by class and records the key design choice of each.
...
\newcommand{\evalstar}{{\color[RGB]{219,16,72}$\star$}}
  ...
  \midrule
  \multirow{6}{=}{\textbf{In-memory}\newline\footnotesize\emph{Fits in one machine's RAM; preprocessing builds a CSR layout}}
  ...
  \midrule
  \multirow{4}{=}{\textbf{Distributed}\newline\footnotesize\emph{Partitioned across machines; communication-bound}}
  ...
  \midrule
  \multirow{7}{=}{\textbf{Out-of-core}\newline\footnotesize\emph{Streamed from disk; preprocessing builds a sequential-I/O layout}}
  ...
  \bottomrule
  \end{tabular}
  \caption{Static graph processing systems. Rows within a class differ in how the layout is packed and traversed; every row shares two limitations: an offline preprocessing step and no mutation during processing. \evalstar{} marks the systems we evaluate \project{} against in~\autoref{sec:eval:static}.}
  \label{tab:static_systems}
  \end{table}

\paragraph{Takeaway:}
First, \textbf{every static system pays a preprocessing tax}.
Each must convert an edge list into its own single-purpose layout before it computes, and that conversion often costs more than the analytics it enables.
\project{} is the only system here that skips it, so we report preprocessing time as a hatched portion of every bar in~\autoref{sec:eval:static} rather than excluding it as one-time setup.
Second, \textbf{every one of them freezes the graph while it computes}.
The layouts in the third column are read-optimized because they assume a frozen graph.
That assumption disqualifies every system in this table from the workloads of~\autoref{sec:eval:dynamic:chapter}, where we compare against a disjoint set of baselines.
Third, \textbf{classes differ in where the graph resides; systems within a class differ primarily in how they manage state to maximize sequential access and minimize random access}.
Only the starred systems return in~\autoref{sec:eval:static}; those are the ones to remember.

The no-mutation limitation admits partial exceptions.
Among the out-of-core systems to which we compare, GraphChi includes an evolving-graph mode that can append edges during execution.
...
\newflag[new: edge- vs vertex-centric table]
\label{design:prog_models}

The \textbf{Programming Model} that we use to express algorithms is orthogonal to the computation model that executes those algorithms.
The programming model defines the interface provided to the user, and therefore, defines the unit of computation in a system.
...
The \textbf{Edge-Centric} model is the other major programming model, in which the user writes a function that operates on each edge of the graph.
...
\begin{table}[t]
    \centering
    \small
    \input{fig/background/vc_ec.tex}
...
\project{}'s adjacency list representation inherits this cost but pays only part of it: \wt{}'s \code{WT\_MODIFY} API (\autoref{sec:wt-modify}) records the appended neighbor ID and the updated degree as a delta rather than rewriting the packed value, which bounds the write amplification but not the contention (\autoref{arch:adjlist}).
