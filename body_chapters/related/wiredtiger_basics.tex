...
\wt{} provides full transactional support with \ac{ACID} guarantees.
...
Other \wt{} metadata, such as session objects, cursor objects, and data handles,  live outside the cache and do not count against \texttt{cache\_size}.
...
\begin{itemize}
  ...
  \item \texttt{eviction\_dirty\_target} / \texttt{eviction\_dirty\_trigger}:
    thresholds applied specifically to the volume of dirty data.
...
The eviction server periodically walks the in-memory B-trees, scoring pages by their \emph{read generation}, which is a counter incremented each time a page is accessed.
It collects candidates, sorts them by read generation, and selects eviction candidates.
These candidates are placed into eviction queues and the eviction worker threads dequeue and process pages concurrently.

Evicting a \emph{clean} (unmodified) page is inexpensive: \wt{} simply
frees the page's memory.
Evicting a \emph{dirty} page first requires \emph{reconciliation}, i.e., converting the page from its in-memory format to the on-disk format, and writing it to storage (see Section~\ref{sec:wt-reconcile}).

%% -------------------------------------------------------------------
\subsection{In-Memory Page Structures: Skip Lists and Update Chains}
...
\begin{itemize}
  ...
  \item The update type: \texttt{STANDARD} (a complete new value),
    ...
    or \texttt{RESERVE}.
  \item A variable-length data payload appended immediately after the fixed structure header.
\end{itemize}
...
The in-memory and on-disk representations serve different purposes.
...
If the page contains uncommitted updates, e.g., from an in-progress transaction, those updates are \emph{restored} after reconciliation so they remain available for the owning transaction to commit or roll back.
