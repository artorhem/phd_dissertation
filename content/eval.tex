...
We start by presenting the research questions that our evaluation answers. 
We then describe the datasets and benchmarks that we use for the evaluation and the experimental setup.
Following that, we present our results to date and the plans to further evaluate the system.
...
These algorithms are widely used in the graph processing community~\cite{mehrotra2024sok} and are representative of the different types of graph algorithms that are used in practice.

\subsection{Modifications to GAPBS}
% \PM{this can be moved to the apppendix}
% We use \ac{GAPBS}~\cite{beamer2015gap} to run the graph algorithms on our graph database.
...
Flexograph ingests data faithfully; we never change node or edge IDs, even though some systems (Teseo~\cite{teseo2021deLeo}, Stinger~\cite{stinger2012ediger}, GraphOne~\cite{graphone2019kumar}, LiveGraph~\cite{zhu2019livegraph}) map user provided vertex IDs to a logical ID used by the system.
...
Instead of allocating our vector and queue classes (used to store the results) with a size equal to the number of nodes, we instead initialize the classes with a buffer large enough to hold $MAX\_NODE\_ID$ number of nodes in the graph.
This is a minor change and does not affect the performance of the \ac{GAPBS} implementation, especially considering our use of transient attributes. 
...
\hline
\textbf{Graphs} &
  \multicolumn{1}{l|}{\textbf{Vertices $|V|$}} &
  \multicolumn{1}{l|}{\textbf{Edges $|E|$}} &
  \multicolumn{1}{l|}{\textbf{\begin{tabular}[c]{@{}l@{}}Uncompressed\\ Size\end{tabular}}} \\ \hline
cit-Patents~\cite{citPatents}                                             & 3.77M   & 16.50M & 268 MB \\ \hline
dimacs9-USA~\cite{FullUSA92:online}                                       & 23.94M  & 57.70M & 940 MB \\ \hline
...
We use the datasets summarized in~\autoref{tab:datasets} for our evaluation.
...
We conducted all experiments on a single socket workstation equipped with an Intel(R) Xeon(R) W-2275 CPU @ 3.30GHz processor.
...
We use the WiredTiger v11.0.0 for the \project{} but set the \wt{} cache size to 10GB. This cache size is large enough for small graphs but small enough that the largest graphs cannot fit in memory.
...
We span the whole grid of graph data management systems shown in~\autoref{fig:distilled_design_space} and must therefore compare ~\project{} to all classes of graph processing systems.

In particular, we compare ~\project{} to the systems mentioned in~\autoref{tab:competitors}.
...
\begin{table}[]
  ...
  \caption{Systems to which we compare to evaluate the performance of ~\project{} on \textbf{structural graphs}. The systems are divided into four categories: Out-of-core(OOC) Analytics, In-memory Analytics, Dynamic graph stores, and Graph Databases.}
...
We plan to use microbenchmarks to evaluate the performance of the EdgeKey and Adjacency List representations and the overheads they introduce relative to a standard CSR representation.

The microbenchmarks cover the following operations:
\begin{enumerate}
  \item \textbf{Edge Insertion/Deletion}: We measure the time taken to insert or delete a single edge in the graph. We will also measure the time taken to insert/delete a batch of edges into the graph. This microbenchmark leverages the Grafaree workload generator to generate random edge insertions and deletions.
  \item \textbf{Vertex Insertion/Deletion}: We measure the time taken to insert or delete a single vertex in the graph. We also measure the time taken to insert/delete a batch of vertices into the graph. This microbenchmark leverages the Grafaree workload generator to generate random vertex insertions and deletions.
  \item \textbf{Vertex Seek}: This measures the time to find the vertex in the graph representation. We measure the time taken to find a vertex by its ID. We compare this to the time taken to find a vertex by its index in the CSR representation.
  \item \textbf{Neighborhood Scan}: This microbenchmark evaluates the time taken to scan the neighborhood of a randomly chosen vertex in the graph. We measure the time taken to scan the neighborhood and normalize it by the number of edges in the neighborhood, getting the average time per edge. We compare this to the time taken to scan the neighborhood in the CSR representation.
\end{enumerate}

The microbenchmarks should give us a good idea of the overhead of using the GraphAPI for accessing an in-memory graph compared to a native in-memory CSR, which allows us to determine the overhead introduced by the use of GraphAPI and \wt{}. 
...
We compared the performance of~\project{} with GraphChi~\cite{kyrola2012graphchi} on a subset of the benchmarks.
...
\multicolumn{2}{|l|}{} &
  ...
  \multicolumn{1}{l|}{\textbf{\ac{GAPBS}}} &
  \multicolumn{1}{l|}{\textbf{GraphChi}} \\ \hline
\multicolumn{1}{|l|}{\multirow{3}{*}{\textbf{PageRank}}} & Algo time                                                    & 0.481 & 0.063 & 2.700 \\ \cline{2-5} 
\multicolumn{1}{|l|}{}                                   & \begin{tabular}[c]{@{}l@{}}Preprocessing\\ Time\end{tabular} & 1.2   & 2.250 & 1.461 \\ \cline{2-5} 
\multicolumn{1}{|l|}{}                                   & Total Time                                                   & 1.681 & 2.313 & 4.161 \\ \hline
\multicolumn{1}{|l|}{\multirow{3}{*}{\textbf{Triangle Counting (exact)}}} &
  Algo time &
  \begin{tabular}[c]{@{}r@{}}Trust Triangle: 3.770\\ Cycle Triangles: 0.067\end{tabular} &
  NA &
  2.448 \\ \cline{2-5} 
\multicolumn{1}{|l|}{}                                   & \begin{tabular}[c]{@{}l@{}}Preprocessing\\ Time\end{tabular} & 1.2   & NA    & 6.172 \\ \cline{2-5} 
\multicolumn{1}{|l|}{} &
  Total Time &
  \begin{tabular}[c]{@{}r@{}}Trust Triangle: 4.970\\ Cycle Triangles: 1.267\end{tabular} &
  NA &
  14.792 \\ \hline
\end{tabular}
\caption{Preliminary benchmark performance data using the PageRank and Triangle Counting algorithms on the cit-Patents graph. The times are reported in seconds. The preprocessing time mentioned for~\project{} is the time it takes to open the connection and warm the cache.}
\label{tab:eval:structural_inmem}
\end{table}

\autoref{tab:eval:structural_inmem} shows the preliminary performance data that we have obtained so far.
The preprocessing time for \project{} is the time taken to open a connection to the graph database, create graph handles, and warm the cache.
Comparing the algorithm run time,~\project{} is slower than \ac{GAPBS}, but including the preprocessing time for both, we are faster than \ac{GAPBS}.
