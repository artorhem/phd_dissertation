...
In ~\autoref{chapter:background}, we discussed the different models used to represent graphs -- property graphs and structural graphs. 
...
Based on these access patterns, we identified requirements for a data structure that can store graphs and discussed the different data structures used by graph data management systems.
...
Our overarching characterization of the design space is defined by two features: when the system manages property graphs or structural graphs and whether the graphs under management are static or dynamic.
...
This requires a pass over the graph and can take significantly longer to finish than the analytics task itself (For example, GraphChi takes longer to preprocess the graph than it takes to run Triangle Counting or PageRank~\autoref{tab:eval:structural_inmem}).
...
These systems are designed around the idea of caching the most frequently accessed data structures in memory and spilling the rest to disk.
...
These two classes of graph processing systems -- in-memory graph processing systems and out-of-core graph processing systems comprise the
\textit{third classification feature} of out design space of graph data management systems.
...
Clearly, the consistency model used by these systems falls along a spectrum (the left-right axis in the right branch of~\autoref{fig:design_space} points in the direction of diminisihing consistency guarantees).
...
% This convergence greatly influenced our design of  \project{}, which provides users with representations that mimic these data structures.
...
\begin{figure*}
    \centering
    \includegraphics[scale=0.45]{content/fig/background/design_grid.pdf}
